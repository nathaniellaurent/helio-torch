...
def reproject_solar_image_hg(
 ...
 # Project back to helioprojective angles from new observer (after rotation)
    ...
    # Optionally, show x_norm with invalids as nan
    fig_xnorm, ax_xnorm = plt.subplots(figsize=(6, 5))
    im_xnorm = ax_xnorm.imshow(x_norm_vis.cpu().numpy(), cmap='coolwarm', vmin=-1, vmax=1)
    ax_xnorm.set_title('x_norm (invalid=nan)')
    plt.colorbar(im_xnorm, ax=ax_xnorm, fraction=0.046, pad=0.04)
    plt.tight_layout()
